...
def test_accuracies_multilabel():
    ...
    # Switch 1<->3
    consensus_switched = np.array([[i[0], i[3], i[2], i[1]] for i in consensus_all.tolist()])
    assert Accuracy.evaluate(real, consensus_all) == 1.0       # [0, 2, 1, 1, 3] vs [0, 2, 1, 1, 3]
    assert Accuracy.evaluate(real, consensus_switched) == 0.4  # [0, 2, 1, 1, 3] vs [0, 2, 3, 3, 1]
    assert LSAccuracy.evaluate(real, consensus_all) == 1.0
    assert LSAccuracy.evaluate(real, consensus_switched) == 1.0
    ...
    # Switch 0<->3, 1<->2
    consensus_switched = np.array([[i[3], i[2], i[1], i[0]] for i in consensus_all.tolist()])
    assert Accuracy.evaluate(real, consensus_all) == 1.0       # [0, 2, 1, 1, 3] vs [0, 2, 1, 1, 3]
    assert Accuracy.evaluate(real, consensus_switched) == 0.0  # [0, 2, 1, 1, 3] vs [3, 1, 2, 2, 0]
    assert LSAccuracy.evaluate(real, consensus_all) == 1.0
    assert LSAccuracy.evaluate(real, consensus_switched) == 1.0
